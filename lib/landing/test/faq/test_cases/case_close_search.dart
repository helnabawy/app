...
void faqCloseSearchTest(FrequentQuestionData frequentQuestionData) {
  int changeCount = 0;
  //using [testWidgets] because it provides context object vice [test]
  testWidgets('case close search in faq', (WidgetTester tester) async {
    await tester.pumpWidget(BlocProvider<FAQCubit>(
      ...
      child: BlocListener<FAQCubit, FAQState>(
        listener: (context, state) {
          ...
          } else if (state is FAQUpdateState && changeCount == 2) {
            frequentQuestionData.closeSearch(context: context);
          } else if (state is FAQUpdateState && changeCount == 3) {
